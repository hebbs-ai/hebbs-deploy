...
| POST | `/v1/contradictions/prepare` | Get pending contradiction candidates | — | `PendingContradiction[]` |
| POST | `/v1/contradictions/commit` | Commit agent-reviewed verdicts | `verdicts` | `ContradictionCommitResponse` |
...
### Contradiction Prepare (agent-driven)

Retrieve pending contradiction candidates that were flagged by the heuristic classifier during ingest. HEBBS has no LLM -- the agent reviews candidates and commits verdicts.
...
### Contradiction Commit (agent-driven)
