...
#### `reflectPrepare(params?: ReflectPrepareParams): Promise<ReflectPrepareResult>`
...
#### `reflectCommit(params: ReflectCommitParams): Promise<ReflectCommitResult>`
...
#### `contradictionPrepare(): Promise<PendingContradiction[]>`

Retrieve pending contradiction candidates flagged by the heuristic classifier during ingest. HEBBS has no LLM -- the agent reviews candidates with its own reasoning.
...
#### `contradictionCommit(verdicts: ContradictionVerdictInput[]): Promise<ContradictionCommitResult>`
